...
export function getQuery(type: 'getCities' | 'getCity'): string {
    if (!queries.has(type)) {
        switch (type) {
            case 'getCities': {
                queries.set(type, select<CityAttr, CityTypes>(['City.ID', { row: 'name', table: 'City' }, 'posX', 'posY', { row: 'name', table: 'CityTypes', as: 'type' }], 'City', [
                ], [
                    { A: 'CityTypes', equation: { A: 'City', AAttr: 'type', B: 'CityTypes', BAttr: 'ID' } }
                ]))
            }
                break
            case 'getCity': {
                queries.set(type, select<CityAttr, CityAttr>(['ID', 'name', 'posX', 'posY', 'type'],
                    'City',
