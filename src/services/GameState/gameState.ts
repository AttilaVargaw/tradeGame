...
import { CityAttr, TradeRouteProps, City, CityPopulationClass, DailyRequirement, DBEvents, IndustrialBuilding, IndustryData, Item, PopulationClass, PopulationData, ResourceChange, WarehouseItem, TradeRoute, CityPositionProperty, TradeRouteAttributes, CityTypes, TradeRouteInsertAttributes, DBEvent, ConvoyAttr, VehicleAttr, VehicleTypeAttr, VehicleTypeInsertAttributes, VehicleType } from './dbTypes'
...
const init = async () => {
    ...
    console.log(select<VehicleTypeAttr>(['name', 'desc', 'ID', 'price'], 'VehicleTypes', [{ A: 'VehicleTypes', AAttr: 'type', value: 'air' }]))

    await Promise.all([
        db.execute(create('TradeRoutes', [
            { name: 'CityA', type: 'INTEGER', references: 'City' },
            { name: 'CityB', type: 'INTEGER', references: 'City' },
            { name: 'name', type: 'TEXT' },
        ])),
        db.execute(create('VehicleTypes', [
            { name: 'name', type: 'TEXT' },
            { name: 'desc', type: 'TEXT' },
            { name: 'price', type: 'REAL' },
            { name: 'type', type: 'TEXT' },
        ])),
        db.execute(insert<VehicleTypeInsertAttributes>('VehicleTypes',
            {
        ...
        db.execute(create('Convoy', [
            { name: 'name', type: 'TEXT' },
            { name: 'posY', type: 'REAL' },
            { name: 'posX', type: 'REAL' },
        ])),
        db.execute(create('Vehicle', [
            { name: 'name', type: 'TEXT' },
            { name: 'type', type: 'TEXT', references: 'VehicleTypes', referencesOn: 'ID' },
            { name: 'posY', type: 'REAL' },
            { name: 'posX', type: 'REAL' },
            { name: 'convoy', type: 'INTEGER', references: 'Convoy', referencesOn: 'ID' },
        ])),
    ])
...
const getVehicleTypes = (type: string = 'air') => {
    return db.select<VehicleType[]>(select<VehicleTypeAttr>(['name', 'desc', 'ID', 'price'], 'VehicleTypes', [{ A: 'VehicleTypes', AAttr: 'type', value: type }]))
}

const getVehicleType = (ID: string) => {
    return db.select<VehicleType[]>(select<VehicleTypeAttr>(['name', 'desc', 'ID', 'price'], 'VehicleTypes', [{ A: 'VehicleTypes', AAttr: 'ID', value: ID }]))
}
...
const getVehiclesOfConvoy = (ID: string | null) => {
    return db.select(select<VehicleAttr[]>(['name', 'type', 'ID'], 'Convoy', [{ A: 'Vehicle', AAttr: 'convoy', value: ID }]))
}

const getTradeRoute = async (ID: string) => {
    const { cityAID, cityBID, name } = (await db.select<{ name: string, ID: string, cityAID: string, cityBID: string, cityAName: string, cityBName: string }[]>(
        select<TradeRouteAttributes>(['cityB.ID as cityBID', 'cityA.ID as cityAID', 'cityA.name as cityAName', 'cityB.name as cityAName', 'TradeRoutes.name', 'TradeRoutes.ID'], 'TradeRoutes', [
            { A: 'TradeRoutes', AAttr: 'ID', value: ID }
        ], [
            { A: 'City', equation: { A: 'CityA', AAttr: 'ID', B: 'TradeRoutes', BAttr: 'cityA', operator: '=' }, as: 'CityA' },
            { A: 'City', equation: { A: 'CityB', AAttr: 'ID', B: 'TradeRoutes', BAttr: 'cityB', operator: '=' }, as: 'CityB' }
        ]).replace('cityA,', 'cityA')
    ))[0]
...
const getTradeRoutesAsGeoJson = async () => {
    const tradeRoutes = await db.select<{ cityAPosX: number, cityBPosX: number, cityAPosY: number, cityBPosY: number, name: string, ID: string, cityAID: string, cityBID: string, cityAName: string, cityBName: string }[]>(
        select<TradeRouteAttributes>(['cityB.ID as cityBID', 'cityA.ID as cityAID', 'cityA.name as cityAName', 'cityB.name as cityAName', 'cityA.posX as cityAPosX', 'cityB.posX as cityBPosX', 'cityA.posY as cityAPosY', 'cityB.posY as cityBPosY', 'TradeRoutes.name', 'TradeRoutes.ID'], 'TradeRoutes', undefined, [
            { A: 'City', equation: { A: 'CityA', AAttr: 'ID', B: 'TradeRoutes', BAttr: 'cityA', operator: '=' }, as: 'CityA' },
            { A: 'City', equation: { A: 'CityB', AAttr: 'ID', B: 'TradeRoutes', BAttr: 'cityB', operator: '=' }, as: 'CityB' }
        ]).replace('cityA,', 'cityA')
    )

    return tradeRoutes.reduce((prev, { cityAPosY, cityAPosX, ID, cityBPosX, cityBPosY, name, cityAID, cityBID }) => {
        prev.features.push({
            geometry: {
                type: 'LineString',
                coordinates: [[cityAPosX, cityAPosY], [cityBPosX, cityBPosY]]
            },
            properties: {
                cities: [cityAID, cityBID],
                name,
...
const addTradeRoute = async (cities: string[]) => {
    ...
    const data = await db.execute(insert<TradeRouteInsertAttributes>('TradeRoutes', { cityA: cities[0], cityB: cities[1], name: `${start.name} ${end.name} route`, }))
...
const getCity = async (ID: string): Promise<City> => {
    const [cityData, classes, warehouse] = await Promise.all([db.select<City[]>(getQuery('getCity'), [ID]), getPopulation(ID), getCityWarehouse(ID)])

    await Promise.all(classes.map(async (e) => e.dailyRequirement = await getCityDailyConsumption(ID, e.ID)))

    return {
        ...cityData[0], classes, industry: {
            industrialBuildings: await getCityIndustrialBuildings(ID),
            aggregatedInputOutput: await getCityIndustrialResourceChanges(ID)
        }, warehouse, fullPopulation: classes.reduce((p, { num }) => p + num, 0)
    }
}

const getNotExistingCityClasses = async (cityID: string) => {
    const [cityPopulationClasses, populationClasses] = await Promise.all([db.select<(CityPopulationClass)[]>(`
    select CPC.ID, CPC.populationClass
        from CityPopulationClass as CPC
        where CPC.city = ?
    `, [cityID]), db.select<(PopulationClass)[]>(`
        select PC.name, PC.ID
            from PopulationClass as PC
    `)])

    return populationClasses.filter((value) => cityPopulationClasses.every(({ ID }) => value.ID !== ID))
}
...
const getNotAvailableItems = async (cityID: string) => {
    const [items, warehouse] = await Promise.all([db.select<(Item)[]>(`
    ...
    return items.filter((value) => warehouse.every(({ ID }) => value.ID !== ID)).map(({ ID, descriptionKey, nameKey }) => ({ ID, descriptionKey, nameKey }))
}
...
const getCityIndustrialBuildingResourceChanges = async (ID: string) => {
    return (await db.select<(ResourceChange & { buildingNum: number })[]>(`
...
const getCityIndustrialResourceChanges = async (ID: string) => {
    const aggregated = await db.select<(ResourceChange & { buildingNum: number })[]>(`
        select I.ID, IBR.num, I.nameKey, IBS.num as buildingNum
            from IndustrialBuildingDailyRequirement as IBR
    ...
    return Object.entries(groupBy(aggregated,'nameKey')).map<ResourceChange>(([key, coll]) => {
        const { descriptionKey, ID, num, buildingNum } = coll[0] as any

        return { num: num * buildingNum, nameKey: key, descriptionKey, ID } as ResourceChange
    })
}
...
const getCityIndustrialBuildings = async (ID: string) => {
    ...
    await Promise.all(industrialBuildings.map(async (e) => {
        const inputOutputData = await getCityIndustrialBuildingResourceChanges(e.ID)
...
const addIndustrialBuildings = (num: number, industrialBuilding: string, city: string) => {
    return db.select<IndustrialBuilding[]>(`
