...
export const RouteLayer: FC<RouteLayerProps> = ({onRouteClick}) => {
    ...
    const [tradeRoutes, setTradeRoutes] = useState<GeoJSON.FeatureCollection<GeoJSON.LineString, TradeRouteProps>>()

    const routeClick = useCallback((ID: string) => () =>{
        onRouteClick(ID)
    }, [])
    ...
    return <> {tradeRoutes && <GeoJSON ref={layerRef} onEachFeature={({ properties: {ID} }, layer) => {
        layer.addEventListener('click', routeClick(ID)).setTooltipContent(tooltip({ permanent: true, interactive: true,  })).toggleTooltip()
    }
    } pathOptions={tradeRouteStyle} data={tradeRoutes} />}
    </>
}
